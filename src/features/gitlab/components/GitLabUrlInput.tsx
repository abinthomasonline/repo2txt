...
interface GitLabUrlInputProps {
  onValidUrl?: (url: string) => void;
  onUrlChange?: (url: string, isValid: boolean) => void;
  hideSubmitButton?: boolean;
}

export function GitLabUrlInput({ onValidUrl, onUrlChange, hideSubmitButton = false }: GitLabUrlInputProps) {
  ...
  const provider = useMemo(() => new GitLabProvider(), []);
  ...
  useEffect(() => {
    ...
    if (isValidUrl) {
      ...
      setError('Invalid GitLab URL format');
    }
      ...
      {showHints && (
        <div className="rounded-md bg-blue-50 dark:bg-blue-900/20 p-3 text-sm space-y-2">
          ...
          <ul className="text-blue-700 dark:text-blue-300 space-y-1 list-disc list-inside">
            ...
            <li>
              <code className="text-xs bg-blue-100 dark:bg-blue-800 px-1 py-0.5 rounded">
                https://self-hosted.com/owner/repo
              </code>
            </li>
          </ul>
        </div>
